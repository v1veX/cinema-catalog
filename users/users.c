...
void reg() {
    char login[21], pass[21], card[17], cont;
    ...
    while (1) {
        ...
        if (login_length < 3 || login_length > 20) {
            print_reg_error("Login must contain from 3 to 20 symbols! Press any key to try again.");
        } else if (is_login_free(login) == 0) {
            print_reg_error("This login is already exists! Press any key to try again.");
        } else if (password_check(pass, pass_length) == 0) {
            print_reg_error("Password must contain at least one number, one capital letter, one small letter! Press any key to try again.");
        } else if (pass_length < 6 || pass_length > 20) {
            print_reg_error("Password must contain from 6 to 20 symbols! Press any key to try again.");
        } else if (card_number_check(card, card_length) == 0) {
            print_reg_error("Card number must contain only numbers! Press any key to try again.");
        } else if (card_length != 16) {
            print_reg_error("Card number must contain 16 numbers! Press any key to try again.");
        } else break;
    }
